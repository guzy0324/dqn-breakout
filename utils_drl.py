...
class Agent(object):
    ...
    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        '''values_next = self.__target(next_batch.float()).max(1).values.unsqueeze(1).detach()'''
        values_next = self.__target(next_batch.float()).gather(1, self.__policy(next_batch.float()).max(1).indices.unsqueeze(1)).detach()
